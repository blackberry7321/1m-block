...
void load_blocked_urls(const char* filename) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        char* token = strtok(line, ","); // This should get the index, which we ignore.
        token = strtok(NULL, ",\n");    // This should get the URL.
        if (token) {
            blocked_urls[num_urls] = strdup(token); // Save the URL after removing the index.
            num_urls++;
        }
    }
...
int hostcheck(char* http_packet) {
    ...
    for (int i = 0; i < num_urls; i++) {
        ...
        if (!reti) {
            printf("Blocked: Match found for %s\n", blocked_urls[i]);
            return 1;
        }
    }
